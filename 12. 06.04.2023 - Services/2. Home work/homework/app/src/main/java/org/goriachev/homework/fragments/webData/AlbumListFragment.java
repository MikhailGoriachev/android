...
import org.goriachev.homework.dialogues.webData.AlbumDialog;
...
import org.goriachev.homework.services.AlbumListService;
...
public class AlbumListFragment extends Fragment implements Serializable {
    ...
    public static final String BROADCAST_ACTION = "org.goriachev.homework.AlbumListFragment";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        var intent = new Intent(getContext(), AlbumListService.class);
    ...
    // вывод элемента
    Consumer<Album> showAlbumItem = (a) -> {
        api.albumsGetById(a.getId()).enqueue(
                new CustomCallback<Album>(
                        (album) -> new AlbumDialog(album.body())
                                .show(getChildFragmentManager(), "album_dialog"),
                        showErrorMessage
                )
        );
    };
    ...
    public void onReceiveHandler(Context context, Intent intent) {
        ...
        if (serviceName.equals(AlbumListService.SERVICE_NAME)) {

            if (resultCode == ResultCode.RESULT_OK)
                rcvItems.setAdapter(new AlbumAdapter(
                        getContext(),
                        (List<Album>) intent.getSerializableExtra("items"),
                        showAlbumItem
                ));
            else
